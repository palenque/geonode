...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'ResourceBase.is_public'
        db.add_column(u'base_resourcebase', 'is_public',
                      self.gf('django.db.models.fields.BooleanField')(default=False),
                      keep_default=False)

        for res in ResourceBase.objects.all():
            anonymous = Group.objects.get(name='anonymous')
            perms = get_groups_with_perms(res.get_self_resource(),attach_perms=True)
            res.is_public = 'view_resourcebase' in perms.get(anonymous,[])
            res.save()

    def backwards(self, orm):
        # Deleting field 'ResourceBase.is_public'
        db.delete_column(u'base_resourcebase', 'is_public')
